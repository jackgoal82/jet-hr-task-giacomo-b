...
- **Mensilità**: il calcolatore permette di scegliere tra 13 mensilità (tredicesima) e 14 (tredicesima + quattordicesima), con **13 come default**, lo standard più diffuso tra i CCNL italiani per impiegati del settore privato in assenza di un CCNL specificato dal task. *Motivo*: il numero di mensilità non incide sulle imposte (IRPEF, INPS e addizionali si calcolano sempre sul reddito annuo), quindi renderlo selezionabile è un'aggiunta a costo pressoché nullo che non tocca la logica fiscale già verificata — cambia solo come il netto annuale viene suddiviso tra le mensilità in busta paga (vedi `nettoMensilitaOrdinaria` e `nettoMeseConMensilitaAggiuntiva` in [src/calculator/types.ts](../src/calculator/types.ts)).
